...
export class ConsoleComponent implements OnInit {
  ...
  delete(console: IConsole): void {
    const modalRef = this.modalService.open(ConsoleDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.console = console;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.consoles = this.refineData(dataFromBody);
  }

  protected refineData(data: IConsole[]): IConsole[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IConsole[] | null): IConsole[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.consoleService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
